...
public class RedisClient {
     ...
     *
     * @param key
     * @param value 过期时间，单位：毫秒
     * @return 成功返回1 如果存在 和 发生异常 返回 0
     */
    public Long pexpire(String key, long value) {
        Jedis jedis = null;
        try {
            jedis = jedisPool.getResource();
            jedis.select(indexdb);
     ...
     * 消费，订阅
     */
    public void subscribe(Serializable sessionId, String nodeId, String chanel) {
        Jedis jedis = null;
        try {
            MyJedisSub myJedisSub = new MyJedisSub(sessionId, nodeId);
            jedis = jedisPool.getResource();
            jedis.subscribe(myJedisSub, chanel);
        } catch (Exception e) {
            log.error(e.getMessage());
        } finally {
            returnResource(jedisPool, jedis);
